...
def _spatial_knn_join(left_df, right_df, left_cols, right_cols, max_rows):
    ...
    # After police_reports⨝accidents, prefer accident POINT over report POINT for nearest hops.
    if "coordinates_ac" in left_sample.columns:
        left_lats, left_lons = _extract_point_coords_series(left_sample["coordinates_ac"])
    else:
...
def _get_join_edge(join_edges, left_table, right_table):
    for edge in join_edges or []:
        if edge.get("from") == left_table and edge.get("to") == right_table:
            return edge
        if edge.get("from") == right_table and edge.get("to") == left_table:
            return edge
    return None


def _relation_should_use_semantic_key(left_df, right_df, join_col):
    matches = semantic_joins.find_semantic_column_matches(
        left_df[[join_col]],
        right_df[[join_col]],
    )
    return any(match.left_col == join_col and match.right_col == join_col for match in matches)
...
def generate_joined_tuples(
    ...
    if len(by_table) == 1:
        ...
        df = semantic_joins.drop_duplicate_columns(data_lake[table_name])
    ...
    if len(tables_list) >= 2:
        merged = semantic_joins.drop_duplicate_columns(data_lake[tables_list[0]])
        joined_any = False

        for i in range(1, len(tables_list)):
            next_table = tables_list[i]
            next_df = semantic_joins.drop_duplicate_columns(data_lake[next_table])
            edge = _get_join_edge(join_edges, tables_list[i - 1], next_table)
            edge_type = edge.get("type") if edge else None
            left_unique = semantic_joins.drop_duplicate_columns(merged)
            right_unique = semantic_joins.drop_duplicate_columns(next_df)
            ...
            if edge_type == "semantic":
                ...
                if semantic_joined is not None:
                    merged = semantic_joined
                    joined_any = True
                    continue

            relation_candidates = []
            if edge and edge_type == "relation" and edge.get("attributes"):
                relation_candidates = [
                    attr.strip()
                ...
                relation_candidates = [
                    ...
                    if attr in left_unique.columns and attr in right_unique.columns
                ]
            if not relation_candidates:
                relation_candidates = sorted(set(left_unique.columns) & set(right_unique.columns))
            if not relation_candidates:
                continue

            join_col = relation_candidates[0]
            if _relation_should_use_semantic_key(left_unique, right_unique, join_col):
                semantic_joined = semantic_joins.semantic_merge(
                    left_unique,
                    right_unique,
                    [(join_col, join_col)],
            ...
            merged = semantic_joins.safe_merge(
                left_unique,
                right_unique,
                on=join_col,
                how='inner',
...
def join_edges_for_path_ordered(
    path_tables,
    rel_edges,
    semantic_edges,
    spatial_edges,
    ...
    """
    Build join edge dicts in path order (from → to along path_tables) for generate_joined_tuples.
    Prefers relation, then semantic, then spatial links for each consecutive pair.
    Returns None if any step has no connecting edge.
    """
    if not path_tables:
        return None
    if len(path_tables) == 1:
        return []

    edges = []
    for i in range(len(path_tables) - 1):
        left, right = path_tables[i], path_tables[i + 1]
        edge = None
        for a, b, label in rel_edges or []:
            if {a, b} == {left, right}:
                edge = {"from": left, "to": right, "type": "relation", "attributes": label or ""}
                break
        if edge is None:
            for a, b, label in semantic_edges or []:
                if {a, b} == {left, right}:
                    edge = {"from": left, "to": right, "type": "semantic", "attributes": label or ""}
                    break
        if edge is None and allow_spatial:
            for a, b, label in spatial_edges or []:
                if {a, b} == {left, right}:
                    edge = {"from": left, "to": right, "type": "spatial", "attributes": label or ""}
                    break
        if edge is None:
            return None
        edges.append(edge)
...
def find_min_join_path(
    ...
    If allow_spatial is False, only relational or semantic edges are allowed.
    ...
    for left, right, label in rel_edges or []:
        _add_edge(left, right, label, "relation")
    for left, right, label in semantic_edges or []:
        _add_edge(left, right, label, "semantic")
    for left, right, label in spatial_edges or []:
