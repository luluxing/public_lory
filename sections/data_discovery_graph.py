...
def _stable_int_inclusive(low: int, high: int, *parts: object) -> int:
    if high <= low:
        return low
    span = (high - low) + 1
    return low + (_stable_hash_int(*parts) % span)


def _sample_visual_spatial_edges(
    spatial_edges: list[dict[str, object]],
    *,
    ...
    visible_edges = [
        ...
        if str(edge.get("source")) in visible_names and str(edge.get("target")) in visible_names
    ]
    if not visible_edges:
        return []

    incident_neighbors: dict[str, set[str]] = {}
    edge_lookup: dict[frozenset[str], dict[str, object]] = {}
    for edge in visible_edges:
        source = str(edge.get("source"))
        target = str(edge.get("target"))
        pair_key = frozenset((source, target))
        incident_neighbors.setdefault(source, set()).add(target)
        incident_neighbors.setdefault(target, set()).add(source)
        edge_lookup[pair_key] = edge

    selections: dict[str, set[str]] = {}
    lake_name = st.session_state.get("chosen_lake") or ""
    for table_name, neighbors in incident_neighbors.items():
        ordered_neighbors = sorted(neighbors)
        neighbor_count = len(ordered_neighbors)
        if neighbor_count == 0:
            selections[table_name] = set()
            continue
        max_sample_size = max(1, neighbor_count // 2)
        sample_size = _stable_int_inclusive(
            1,
            max_sample_size,
            lake_name,
            "ddg_visual_spatial_edge_count",
            table_name,
            tuple(ordered_neighbors),
        )
        ...
        selections[table_name] = set(ranked_neighbors[:sample_size])

    # For undirected joins, treat a single endpoint's selection as a shared table-pair choice.
    for edge in visible_edges:
        ...
        target = str(edge.get("target"))
        if (
            target in selections.get(source, set())
            or source in selections.get(target, set())
        ):
            selections.setdefault(source, set()).add(target)
            selections.setdefault(target, set()).add(source)

    sampled_edges: list[dict[str, object]] = []
    for edge in edge_lookup.values():
        source = str(edge.get("source"))
        target = str(edge.get("target"))
        source_selected_target = target in selections.get(source, set())
        target_selected_source = source in selections.get(target, set())

        if source_selected_target or target_selected_source:
...
<div style="display:flex;flex-wrap:wrap;gap:12px 20px;align-items:center;margin:0.15rem 0 0.85rem 0;font-size:13px;color:#333;">
  ...
  <span style="display:inline-flex;align-items:center;gap:6px;">
    <span style="display:inline-block;width:28px;height:0;border-bottom:3px solid #2563eb;vertical-align:middle;"></span>
    Semantic join
  </span>
  <span style="display:inline-flex;align-items:center;gap:6px;">
    <svg width="32" height="8" style="vertical-align:middle" aria-hidden="true">
      <line x1="0" y1="4" x2="32" y2="4" stroke="#dc2626" stroke-width="3"/>
    </svg>
    Spatial join
  </span>
</div>
...
def _build_graphviz(nodes, rel_edges, spatial_edges) -> str:
    ...
    for left, right, label in spatial_edges:
        label_txt = (
            f' [label="{label}", color="#dc2626", fontcolor="#dc2626", style="solid"]'
        )
        lines.append(f'  "{left}" -- "{right}"{label_txt};')
...
def _d3_discovery_graph_html(d3_data: dict) -> str:
    ...
    function linkPath(d) {{
      ...
      const endPadding = d.type === 'spatial' && d.directed ? 28 : 26;
...
def render_data_discovery_graph_section(
    ...
    spatial_edges_visual = _sample_visual_spatial_edges(
        spatial_edges_full,
        visible_names=visible_names,
    )

    graph_edges = base_edges + [dict(edge) for edge in spatial_edges_visual]
    graph_edges = [
        e
    ...
    edges_data = base_edges + [dict(edge) for edge in spatial_edges_full]
    edges_data = [
        e
        for e in edges_data
        if e["source"] in visible_names and e["target"] in visible_names
    ...
    component_event = DDG_COMPONENT(
        graph_html=_d3_discovery_graph_html({"nodes": nodes_data, "links": graph_edges}),
        height=540,
        default=None,
