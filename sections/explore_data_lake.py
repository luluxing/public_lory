        ...
        if isinstance(val, str):
            ...
            if "," in txt and "POINT" not in txt.upper():
                ...
                if len(parts) >= 2:
                    ...
                    return (lat, lon)
            point = extract_point_coords(txt)
            if point:
                return point
    except Exception:
        return None
...
def _parse_label_columns(label: str):
    ...
    for part in label.replace("…", "").split(","):
        ...
        if not item:
            continue
        if "≈" in item:
            left_col, right_col = [piece.strip() for piece in item.split("≈", 1)]
            cols.extend([left_col, right_col])
        else:
            cols.append(item)
...
def render_explore_section():
    ...
    if active_bbox:
        ...
        if overlaps:
            ...
            adj = defaultdict(set)
            edge_info = {}
            for left, right, _ in rel_edges + semantic_edges + spatial_edges:
                adj[left].add(right)
                adj[right].add(left)
            for left, right, label in rel_edges:
                edge_info.setdefault((left, right), ("rel", label))
                edge_info.setdefault((right, left), ("rel", label))
            for left, right, label in semantic_edges:
                edge_info[(left, right)] = ("semantic", label)
                edge_info[(right, left)] = ("semantic", label)
            ...
            for idx, path in enumerate(paths_state):
                ...
                with col1:
                    ...
                    for pos in range(len(path) - 1):
                        join_type, join_label = edge_info.get(
                            (path[pos], path[pos + 1]),
                            ("rel", ""),
                        )
                        path_data["joins"].append(
                            {
                                "from": path[pos],
                                "to": path[pos + 1],
                                "attributes": join_label,
                                "type": "relation" if join_type == "rel" else join_type,
                            }
                        )
                    render_join_path_card(
                        component_id=f"explore_path_{idx}_card",
                        title=f"Join Path {idx + 1:03d}",
                        path_data=path_data,
                    ...
                    for pos, tbl in enumerate(path):
                        if pos == 0:
                            ...
                            join_kind = "relational" if jtype == "rel" else "semantic" if jtype == "semantic" else "spatial" if jtype == "spatial" else "join"
