...
def drop_duplicate_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Keep the first occurrence of each column label."""
    return df.loc[:, ~df.columns.duplicated()]


def _as_column_list(columns) -> list:
    if columns is None:
        return []
...
def _next_available_column_name(base_name, taken: set, suffix: str) -> str:
    candidate = f"{base_name}{suffix}" if suffix else str(base_name)
    if candidate not in taken:
        return candidate

    counter = 2
    while True:
        candidate = f"{base_name}{suffix}{counter}" if suffix else f"{base_name}_{counter}"
        if candidate not in taken:
            return candidate
        counter += 1
...
def safe_merge(
    ...
    *,
    ...
    """Merge while avoiding suffix collisions on overlapping right-side columns."""
    left_prepared = drop_duplicate_columns(left_df)
    right_prepared = drop_duplicate_columns(right_df)
    _, right_suffix = suffixes

    join_keys = set(_as_column_list(on) or _as_column_list(right_on))
    taken_names = set(left_prepared.columns) | set(right_prepared.columns)
    rename_map = {}

    for column in right_prepared.columns:
        if column in join_keys:
            continue
        if column not in left_prepared.columns:
            continue

        reserved_names = taken_names - {column}
        new_name = _next_available_column_name(column, reserved_names, right_suffix)
        rename_map[column] = new_name
        taken_names.add(new_name)

    if rename_map:
        right_prepared = right_prepared.rename(columns=rename_map)
...
def semantic_merge(
    ...
    left_tmp = drop_duplicate_columns(left_df).copy()
    right_tmp = drop_duplicate_columns(right_df).copy()
    left_keys: list[str] = []
    right_keys: list[str] = []
