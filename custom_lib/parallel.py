...
class TqdmThreaded(threading.Thread):
    def __init__(self,
        ...
        print(f"tqdm info: process count = {self.process_count}, multiplier of update = {self.multiplier}")
...
def _execute_serial(func, argument_list, common_arguments, function_for_batch):
    results = []
...
def execute(func, argument_list, common_arguments={}, function_for_batch=None, parallel_enabled=True):
    print(str(func), f"parallel enabled: {parallel_enabled}")
    if function_for_batch is None:
        function_for_batch = empty_function

    cmd_arguments = parse_arg([
    ...
    if not parallel_enabled:
        return _execute_serial(func, argument_list, common_arguments, function_for_batch)
    # ---------------------------------------------------------------------
    ...
    process_count = np.min([cpu_count(), process_count])
    ...
    try:
        ...
        print(f"Parallel setup failed, falling back to serial execution: {e}", file=sys.stderr)
        return _execute_serial(func, argument_list, common_arguments, function_for_batch)
    ...
    try:
        with Pool(processes=process_count, initializer=init) as p:
            ...
            try:
                ...
                print(f"Parallel execution failed, falling back to serial execution: {e}", file=sys.stderr)
                traceback.print_exc()
                p.terminate()
                p.join()
